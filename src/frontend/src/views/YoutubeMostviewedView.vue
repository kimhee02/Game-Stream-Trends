<template>
  <div class="youtube-container">
    <header class="youtube-header">
      <h1 class="title">Youtube Most Viewed Videos</h1>
      ...
      <table class="stats-table">
        <thead>
          <tr>
            ...
            <th @click="sortTable('curViewcount')">
              View Counts
              <span class="sort-icon" :class="getSortIcon('curViewcount')"></span>
            </th>
            <th @click="sortTable('ratioViewcount')">
              Increase
              <span class="sort-icon" :class="getSortIcon('ratioViewcount')"></span>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(game, index) in sortedGames" :key="game.rank">
            ...
            <td class="cur">
              {{ formatNumber(game.curViewcount) }}
              <br>
              <span class="change-positive">(+{{ formatNumber(game.changeViewcount) }})</span>
            </td>
            <td class="ratio">
              <span :class="getRatioClass(game.ratioViewcount)">
                {{ game.ratioViewcount }}%
              </span>
            </td>
          </tr>
...
export default {
  name: "YoutubeMostviewedView",
  data() {
    return {
      games: [], // Original games data
      sortKey: "curViewcount", // Default column for sorting
  ...
  methods: {
    async fetchData() {
      try {
        const response = await axios.get("http://43.202.154.13:8000/api/youtube/view");
        ...
        this.games = rawData.map((game, index) => ({
          ...
          channelName: game[3],
          curViewcount: game[4],
          changeViewcount: parseInt(game[6].replace(/[+,]/g, ""), 10),
          ratioViewcount: parseInt(game[7]) === 0 ? 100 : parseInt(game[7])
        }));
      } catch (error) {
