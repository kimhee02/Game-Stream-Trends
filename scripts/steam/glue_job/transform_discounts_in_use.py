...
from pyspark.sql.functions import col, lit, regexp_extract, input_file_name, to_timestamp, when, create_map, explode

KST = pytz.timezone('Asia/Seoul')

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("gureum-steam-discounts-in-use-glue-job-logger")
...
try:
    ...
    while current_time <= end_time:
        try:
            ...
            default_timestamp = last_modified_str if last_modified_str else f"{formatted_date}_00-00-00"
            ...
            for key in top_level_keys:
                try:
                    logger.info(f"Processing app_id: {key}")
                    ...
                    data_field = next((f for f in raw_data.schema[key].dataType.fields if f.name == "data"), None)
                    if data_field:
                        data_type = data_field.dataType.simpleString()
                        logger.info(f"Data type for app_id {key}: {data_type}")
                        
                        if data_type in ["string", "array<string>"]:
                            logger.info(f"Standardizing non-struct data type to None for app_id: {key}")
                            raw_data = raw_data.withColumn(f"`{key}`.data", lit(None).cast(data_schema))
                        elif not data_type.startswith("struct"):
                            logger.warning(f"Unsupported data type for app_id {key}: {data_type}. Standardizing to None.")
                            raw_data = raw_data.withColumn(f"`{key}`.data", lit(None).cast(data_schema))
                    
                    logger.info(f"Ensuring price_overview structure for app_id: {key}")
                    raw_data = raw_data.withColumn(
                        f"`{key}`.data.price_overview",
                        when(col(f"`{key}`.data").isNotNull() & col(f"`{key}`.data.price_overview").isNotNull(), col(f"`{key}`.data.price_overview"))
                        .otherwise(lit(None).cast(price_overview_schema))
                    )
                    
                    for field in ["currency", "initial", "final", "discount_percent"]:
                        column_name = f"{field}_{key}"
                        raw_data = raw_data.withColumn(
                            column_name,
                            when(col(f"`{key}`.data.price_overview.{field}").isNotNull(), col(f"`{key}`.data.price_overview.{field}"))
                            .otherwise(lit(None))
                        )
                        logger.info(f"Standardized column: {column_name}")

                except Exception as e:
                    logger.error(f"Error processing app_id {key}: {str(e)}")
                    continue

            try:
                logger.info("Standardized schema after processing:")
                raw_data.printSchema()
                logger.info("Sample data after processing:")
                raw_data.show(5, truncate=False)
            except Exception as e:
                logger.error(f"Error logging standardized schema or sample data: {str(e)}")

            logger.info("Preparing data for parallel processing...")
            standardized_columns = [
                f"struct('{key}' as app_id, `{key}`.data.price_overview.currency as currency, "
                f"`{key}`.data.price_overview.initial as initial_price, "
                f"`{key}`.data.price_overview.final as final_price, "
                f"`{key}`.data.price_overview.discount_percent as discount_percent, "
